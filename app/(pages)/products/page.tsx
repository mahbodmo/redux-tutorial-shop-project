...
export default async function ProductsPage({ searchParams }: Params) {
  const { limit = DEFAULT_PAGINATION_LIMIT, productName } = await searchParams;

  try {
    const response = await fetch(
      urlWithQueryParams(
        `https://dummyjson.com/products${productName ? "/search" : ""}`,
        {
      ...
      <>
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-baseline gap-x-2">
            <SortButton />
            <FilterButton productNameQuery={productName} />
          </div>
        </div>
